...
class FileSource extends BaseSource
{
    ...
    public function __construct($basePath = '/', $extension = '.html')
    {
    ...
    public function setExtension($extension)
    {
        if (! is_string($extension)) {
            throw new \InvalidArgumentException('Extension is not a string');
        }
        if (! in_array($extension, [
            '.html',
        ])
        ) {
            throw new \InvalidArgumentException('Invalid extension supplied');
        }
    ...
    {
        return $this->addParamsToUrl($this->getBasePath(), $params) . $this->getExtension();
    }
